...
public class Robot {
	...
	public void findPath(int startX, int startY, int endX, int endY) {
		...
		while (isEmpty == false && isFound == false) {
			...
			System.out.println("꺼냄 : " + curPos);
			while (isFound == false && direction < NUMDIRECTIONS) {
				
				/*
				 * ## 유의사항
				 * 메트릭스(미로) 위치를 표시하는 Move의 x,y 는 좌표 개념이며
				 * 2차원 배열에 담는 값은 행열 개념이므로 위의 x값과 y값이 코드상 교차된다. 
				 */
				...
				if (newX >= 0 && newX < WIDTH 
					...
					newPosition.direction = direction + 1; //앞서 가본 방향을 제외한 다음 방향설정
